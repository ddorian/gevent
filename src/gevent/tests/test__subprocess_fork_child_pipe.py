...
theft running the other way. There a copy of a parent greenlet resumed in the
pre-exec child and *wrote*; here it *reads*, and the bytes it takes off an
inherited pipe never reach the parent that was capturing them.

On a TLS socket that theft is loud, because the record layer's sequence numbers
desynchronize and the peer answers ``bad_record_mac``. An anonymous pipe has no
MAC, so it is silent: the capture is simply short, and in the worst case empty,
which is how it was first seen in production (an ``ffmpeg`` run whose combined
stdout and stderr came back as zero bytes with the producer exiting 0).
...
#: Concurrent producers. One is enough to see the defect, but only on about
#: half of the runs; every extra pipe is another chance for a given fork window
#: to land while a reader is parked on it, and at this many the unfixed
#: behaviour showed up on every attempt.
PRODUCERS = 16
SPAWNS = 120
