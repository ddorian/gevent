...
Only the greenlet that forked survives a fork, the same way only the calling
thread survives one. Everything else in the parent's hub is a copy, and if the
child resumes one it redoes work the parent has already done or is doing: a
second INSERT, a second write on an inherited socket.

The child does not have to *do* anything for that to happen.
``os.register_at_fork(after_in_child=)`` handlers run inside ``fork()`` itself,
before it returns to whoever called it, and under monkey-patching the ordinary
contents of such a handler yield: ``Thread.start()`` waits on an ``Event``, a
patched lock may be held by another greenlet, ``logging`` takes one. That yield
hands control to the child's copy of the hub, which resumes whatever was queued.
"""
