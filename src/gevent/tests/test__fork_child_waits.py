...
:mod:`gevent.tests.test__fork_child_greenlets` covers the copies that were
*runnable*, which sit on the loop's callback queue. A greenlet that was blocked
is not there: it is parked on a watcher, and the loop resumes it from the
watcher without consulting that queue. Both kinds are copies in the child and
neither may run.

The workers here sleep between writes, so at any fork most of them are parked on
a timer rather than queued. The child does nothing but ``os._exit(0)``, so every
line bearing a child's pid was written by a copy of somebody else's greenlet,
